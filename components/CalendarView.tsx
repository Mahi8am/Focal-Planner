        ...
        return (
          <Animated.Text
            ...
            style={{
              ...
              fontWeight: "600",
              color,
              opacity: anims[i]?.op ?? 1,
              transform: [{ translateY: anims[i]?.y ?? 0 }, { rotate: rot }],
            }}
          >
            {char}
          </Animated.Text>
    ...
    <Animated.View
      style={[
        styles.cellOuter,
        {
          opacity,
          transform: [
            { scale: Animated.multiply(scaleAnim, bounce) },
            { rotate },
          ],
        },
      ]}
      ...
      >
        <Text
          style={[
            ...
            (isTodayCell || isSelected) && { fontWeight: "900" },
          ]}
        >
          {day}
        </Text>
...
export default function CalendarView({ data, onGoToDay, colors }: Props) {
  ...
  const changeMonthFn = useCallback(
    (
      newYear: number,
      newMonth: number,
      prevYear: number,
      ...
      const newSelected = isCurrentMonth
        ? today
        : toDateKey(new Date(newYear, newMonth, 1));
  ...
  useEffect(() => {
    changeMonthRef.current = changeMonthFn;
  }, [changeMonthFn]);
  const animateDetailRef = useRef(animateDetailChange);
  useEffect(() => {
    animateDetailRef.current = animateDetailChange;
  }, [animateDetailChange]);

  const prevMonth = useCallback(() => {
    const y = viewYearRef.current,
      m = viewMonthRef.current;
  ...
  const navigateSelectedDay = useCallback((dir: number) => {
    ...
    if (
      cur.getMonth() !== viewMonthRef.current ||
      cur.getFullYear() !== viewYearRef.current
    ) {
      changeMonthRef.current(
        cur.getFullYear(),
        cur.getMonth(),
        viewYearRef.current,
        viewMonthRef.current,
      );
    }
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            <Text style={[styles.pickerTitle, { color: colors.textMuted }]}>
              SELECT YEAR
            </Text>
            <ScrollView
              style={{ maxHeight: 280 }}
            ...
            >
              {years.map((y) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.pickerItem,
                    y === viewYear && { backgroundColor: colors.red },
                  ]}
                  onPress={() => {
                    changeMonthFn(y, viewMonth, viewYear, viewMonth);
            ...
            >
              {MONTHS.map((m, i) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.pickerItem,
                    i === viewMonth && { backgroundColor: colors.red },
                  ]}
                  onPress={() => {
                    changeMonthFn(viewYear, i, viewYear, viewMonth);
      ...
      >
        ...
        <View style={styles.monthYearRow}>
          <TouchableOpacity
            ...
            style={[styles.monthBtn, { borderColor: colors.red }]}
            activeOpacity={0.7}
          >
            <Animated.Text
              style={[
                styles.monthBtnText,
                { color: colors.text, transform: [{ scale: monthScale }] },
              ]}
            >
          ...
          >
            <Animated.Text
              style={[
                styles.monthBtnText,
                { color: colors.red, transform: [{ scale: yearScale }] },
              ]}
            >
              {viewYear}
            </Animated.Text>
      ...
      <View style={styles.grid} {...gridPan.panHandlers}>
        {cells.map((day, idx) => {
          if (!day)
            return (
              <View key={`e-${idx}-${gridKey}`} style={styles.cellOuter} />
            );
      ...
      <View style={[styles.legend, { borderTopColor: colors.border }]}>
        ...
        <View style={styles.legendItem}>
          <View
            style={[styles.legendDot, { backgroundColor: colors.failed }]}
          />
          <Text style={[styles.legendText, { color: colors.textMuted }]}>
            Failed
          </Text>
      ...
      >
        ...
        >
          <View
            style={[styles.dayDetailStripe, { backgroundColor: colors.red }]}
          />
          <View style={styles.dayDetailInner}>
            <View style={styles.dayDetailHeader}>
              ...
              <TouchableOpacity
                style={[styles.goBtn, { backgroundColor: colors.red }]}
                onPress={() => onGoToDay(selectedDateKey)}
                activeOpacity={0.8}
              >
            ...
            {SLOTS.map((slot, i) => {
              ...
              return (
                ...
                >
                  ...
                  {isCompleted && (
                    <CheckCircle size={16} color={colors.completed} />
                  )}
